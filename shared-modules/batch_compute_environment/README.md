...
|------|-------------|------|---------|:--------:|
| <a name="input_allocation_strategy"></a> [allocation\_strategy](#input\_allocation\_strategy) | The allocation strategy to use for the compute environment | `string` | `"BEST_FIT"` | no |
| <a name="input_bid_percentage"></a> [bid\_percentage](#input\_bid\_percentage) | The bid percentage to use for the compute environment | `number` | `0` | no |
...
| <a name="input_ec2_key_pair"></a> [ec2\_key\_pair](#input\_ec2\_key\_pair) | The EC2 key pair to use for the compute environment | `string` | `null` | no |
| <a name="input_image_id_override"></a> [image\_id\_override](#input\_image\_id\_override) | The image ID to use for the compute environment | `string` | `null` | no |
...
| <a name="input_name"></a> [name](#input\_name) | Name of the compute environment | `string` | n/a | yes |
| <a name="input_partition_count"></a> [partition\_count](#input\_partition\_count) | The number of partitions to use for the compute environment | `number` | `2` | no |
...
| <a name="input_service_role_arn"></a> [service\_role\_arn](#input\_service\_role\_arn) | ARN of the IAM role that Batch uses to manage resources on your behalf. | `string` | `null` | no |
| <a name="input_spot_iam_fleet_role_arn"></a> [spot\_iam\_fleet\_role\_arn](#input\_spot\_iam\_fleet\_role\_arn) | ARN of the IAM role that Batch uses to launch instances in the compute environment. | `string` | `null` | no |
| <a name="input_spread_level"></a> [spread\_level](#input\_spread\_level) | The spread level to use for the compute environment | `string` | `"rack"` | no |
| <a name="input_state"></a> [state](#input\_state) | The state of the compute environment | `string` | `"ENABLED"` | no |
| <a name="input_strategy"></a> [strategy](#input\_strategy) | The placement strategy to use for the compute environment | `string` | `"cluster"` | no |
