...
| <a name="input_global_secondary_indexes"></a> [global\_secondary\_indexes](#input\_global\_secondary\_indexes) | The global secondary indexes of the DynamoDB table | <pre>list(object({<br/>    name = string<br/>    key_schema = list(object({<br/>      attribute_name = string<br/>      attribute_type = optional(string, "S")<br/>      key_type       = string<br/>    }))<br/>    read_capacity      = optional(number, 1)<br/>    write_capacity     = optional(number, 1)<br/>    non_key_attributes = optional(list(string), [])<br/>    projection_type    = optional(string, "INCLUDE")<br/>  }))</pre> | `[]` | no |
